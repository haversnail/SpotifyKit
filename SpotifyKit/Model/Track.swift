...
extension SKTrack {
    ...
    /// Creates and returns the request used to get audio features for the given track.
    ///
    /// - Returns: The `SKRequest` object, for testing purposes.
    internal func makeAudioFeaturesRequest() -> SKRequest {
        return SKRequest(method: .GET, endpoint: Constants.Endpoints.audioFeaturesForTrack(id: id))!
    }
    
    /// Get audio feature information for the given track.
    ///
...
extension Array where Element == SKTrack {
    ...
    /// Creates and returns the request used to get audio features for several tracks.
    ///
    /// - Returns: The `SKRequest` object, for testing purposes.
    internal func makeAudioFeaturesRequest() -> SKRequest {
    ...
    ///
    /// - Parameter handler: The callback handler for the underlying request. The parameters for this handler are:
    ///     - `features`: An array of audio features objects corresponding to the tracks in this sequence. If a particular track cannot be found based on its ID, the resulting array will contain a `nil` value at the corresponding index.
    ///     - `error`: An error object identifying if and why the request failed, or `nil` if the request was successful.
    public func getAudioFeatures(handler: @escaping ([SKAudioFeatures?]?, Error?) -> Void) {
        makeAudioFeaturesRequest().perform(handler: handler)
    }
